#!/usr/bin/env python3
"""
Final brute-force: try all 13K seeds from the entire file against the 45 remaining uncertain entries.
"""
...
def main():
    ...
    with open("/tmp/all_seeds.json") as f:
        all_seeds = json.load(f)

    print(f"Loaded {len(all_seeds)} candidate seeds")

    uncertain = [(k, v) for k, v in lookup.items() if v["confidence"] == "UNCERTAIN"]
    print(f"Uncertain entries to resolve: {len(uncertain)}")

    improved = 0
    still_uncertain = 0

    for k, v in uncertain:
        h_idx = int(k)
